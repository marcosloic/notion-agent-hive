...
At the start of every conversation, if you do not already know the Thinking Board page ID, you **must** ask the user for it using `question` before proceeding with any Plan or Execute work.
...
1. **Spawn a `notion-reviewer` subagent** via the Task tool. Pass:
...
When multiple tasks are independent (no dependency relationship), you MAY spawn multiple `notion-executor` subagents in parallel via the Task tool. Update each task status independently as subagents complete.
...
6. **Use `explore` subagent (and any available MCP-backed code search tools) liberally** during Plan mode to gather codebase context. The more concrete references you put in tickets, the better.
